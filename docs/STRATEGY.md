...
| 2026-07-22 | **🔴 DATA FIX: SPY price history extended 2021-07 → 2018-01** (re-ingested via Robinhood `get_equity_historicals`, merged into `data_cache/SPY_daily.parquet`). Every SPY comparison in the project prior to this measured only 2021-07+ (all the cached SPY there was). Over the corrected full 2018–2025 window SPY does 11.9% CAGR / 34.1% max DD / Sharpe 0.47. | corrects an implicit apples-to-oranges benchmark window in every prior run |
| 2026-07-22 | **❌ TESTED AND NOT ADOPTED: underperformance-triggered recycling valve.** Initially appeared to win the champion-cell three-way test — but that was an ARTIFACT of the SPY-data bug above (the trigger was blind to pre-2021 holds). Re-run on corrected SPY data it is the **WORST** of the three (return÷DD 25.60 vs ~30.0 for no-valve/underwater): it evicts good-enough holdings that merely lag SPY (UBS +23%, MMM +21%) for replacements that did worse (WDC −32%, RCL −55%). "Lagging SPY for a year" is a bad eviction signal. Recycling in general does not earn its keep here (no-valve ties underwater). Neither valve captures the 2022 mega-caps — that is LEAP-slot contention, not equity. No active rule changed; both triggers' code + tests remain. Full detail `reports/exit_entry_valve.md`. | supersedes the same-day "pending adoption" note, which was based on the pre-data-fix numbers |
| 2026-07-22 | **Part 1 diagnostics (no rule change):** trailing mechanic ut_trail vs pct_trail is a wash (pct marginally ahead, ut won 2/6 pairs); daily vs 3-day is a sizing interaction not a standalone edge; the 0.5–0.9 "dead zone" stall is empirically rare (3% of equity trades pooled, zero in the champion cell). See `reports/exit_entry_valve.md`. | none — diagnostic aggregation of the existing 12-cell run |
