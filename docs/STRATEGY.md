...
| 2026-07-22 | **⏸️ PENDING OWNER ADOPTION (not promoted): underperformance-triggered recycling valve.** A new valve trigger (recycle a held equity trailing SPY by ≥5% annualized over its own hold window, vs the current "underwater" trigger) won the champion cell three-way test (return 1221% / maxDD 47.2% / ret÷DD 25.89, vs underwater 1064%/48.2%/22.07 and no-valve 1041%/47.5%/21.91) and recycled the RIGHT targets (lagging mediocre winners like VZ, which the underwater trigger cannot see). BUT two blockers keep it from promotion: (a) it does NOT capture the Oct–Dec 2022 mega-caps it was built for — those are LEAP-slot contention, which this equity-only valve never touches; (b) cached SPY history starts 2021-07, so the trigger is blind to pre-2021 (2020-vintage) holds, making its win partial-coverage. Config default stays `slot_recycling.recycle_trigger`-equivalent = "underwater" until the owner adopts. Code+tests live (`recycle_trigger="underperformance"`); full detail `reports/exit_entry_valve.md`. | does NOT change any active rule — the underwater valve remains the built default; this logs a tested-but-unadopted alternative |
